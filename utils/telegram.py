...
class Accounts:
    ...
    def pars_sessions(self):
        ...
        logger.info(f"找到session: {len(sessions)}!")
        return sessions

    async def check_valid_sessions(self, sessions: list):
        logger.info(f"检查session的有效性!")
        valid_sessions = []
        if config.USE_PROXY:
            proxy_dict = {}
            ...
            for session in sessions:
                try:
                    ...
                    logger.error(f"{session}.session is invalid")
            logger.success(f"有效session: {len(valid_sessions)}; 无效session: {len(sessions)-len(valid_sessions)}")
    ...
    async def get_accounts(self):
        ...
        if not accounts:
            raise ValueError("没有找到有效的session会话")
        else:
            return accounts
